  ...
  function createResults() {
    ...
    $.ajax({
          ...
          jsonp: "jsonCallback",
					timeout: 4000,
          error: function(msg) {console.debug(msg);},
          success: function(data) {
                ...
                $('div.list_results').append('<div class="month '+date.month+'">'+
      						...
      						'<ul>'+
      							'<li>'+
      								...
      								'<img src="'+theme_url+'/images/escudos/'+result[i][1]+'" title="'+result[i][0]+'" alt="'+result[i][0]+'"/><span class="result"><label>'+result[i][0]+'  <strong> '+result[i][7]+' </strong>  '+result[i][2]+'</label></span><img src="'+theme_url+'/images/escudos/'+result[i][3]+'" title="'+result[i][2]+'" alt="'+result[i][2]+'"/>'+
            ...
            $('span.bottom div.next_match span img:eq(0)').attr('src',theme_url+'/images/escudos/'+result[last][1]);
            $('span.bottom div.next_match p:eq(2)').text(last_score[0]);
            $('span.bottom div.next_match p:eq(3)').text(last_score[1]);
            $('span.bottom div.next_match span img:eq(1)').attr('src',theme_url+'/images/escudos/'+result[last][3]);
    ...
    $.ajax({
          ...
          success: function(data) {
            try {
              ...
              $('span.top div.next_match span img:eq(0)').attr('src',theme_url+'/images/escudos/'+results[0][1]);
              $('span.top div.next_match span img:eq(1)').attr('src',theme_url+'/images/escudos/'+results[0][3]);
