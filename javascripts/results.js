  ...
  $(document).ready(function(){
    //Get next match
		...
		sql="SELECT * FROM 5167826 WHERE Date < '"+today+"' ORDER BY Date ASC";
    $.ajax({
                ...
                $('div.list_results').append('<div class="month '+date.month+'">'+
      						...
      						'<ul>'+
      							'<li>'+
      								...
      								'<img src="'+theme_url+'/images/escudos/'+result[i][1]+'" title="'+result[i][0]+'" alt="'+result[i][0]+'"/><span class="result"><label>'+result[i][0]+'  <strong> '+result[i][10]+' </strong>  '+result[i][2]+'</label></span><img src="'+theme_url+'/images/escudos/'+result[i][3]+'" title="'+result[i][2]+'" alt="'+result[i][2]+'"/>'+
